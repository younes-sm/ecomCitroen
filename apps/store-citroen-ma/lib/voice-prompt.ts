// Assembles the system prompt + greeting + agent voice settings for a brand.
// Single source of truth used by BOTH /api/rihla/system-prompt (GET, widget
// display + chat-side consumers) and /api/rihla/voice/token (POST, where the
// prompt is baked into the ephemeral live-session token). Keep them identical
// — a drift between what the widget shows and what the token enforces is
// exactly the class of bug that broke voice on 2026-08-27.
...
export type VoicePromptBundle = {
  systemPrompt: string;
  opening: string;
  voiceName: string;
  brand: { slug: string; name: string; agentName: string };
  locale: Locale;
};

export async function composeVoicePrompt(opts: {
  brandSlug?: string | null;
  localeParam?: string | null;
  voice: boolean;
}): Promise<VoicePromptBundle> {
  const brandSlug = opts.brandSlug ?? "jeep-ma";
  const localeParam = opts.localeParam ?? null;
