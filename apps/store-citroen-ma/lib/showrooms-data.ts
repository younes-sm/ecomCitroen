// Single source of truth for demo showroom data.
//
// Used by:
//   - scripts/seed-showrooms.ts  → seeds these rows into Supabase
//   - app/api/rihla/showrooms     → local fallback when Supabase is
//     unreachable/restricted, so the showroom selector still renders offline.
//
// Jeep note: the RÉSEAU DES MAISONS block in
// app/api/rihla/system-prompt/route.ts must stay in sync with the jeep-ma list
// below, otherwise the voice and chat agents will quote different coverage.

export type ShowroomSeed = {
  name: string;
  city: string;
  address: string;
  phone: string;
...
export const SHOWROOMS_DATA: Record<string, ShowroomSeed[]> = {
  "citroen-ma": [
  ...
  // block in app/api/rihla/system-prompt/route.ts (les deux doivent rester
  // synchronisés sinon la voix et le chat divergent).
