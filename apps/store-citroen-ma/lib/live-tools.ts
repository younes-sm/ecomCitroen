// Tool declarations for the Gemini Live voice session.
//
// PLAIN module — no "use client" directive. It is imported BOTH by the
// server-side ephemeral-token mint (app/api/rihla/voice/token) and by any
// client code that needs the declarations. Keep it dependency-free:
// importing server- or client-only modules here would break a consumer.
//
// These declarations are baked into the ephemeral token's
// bidiGenerateContentSetup at mint time. On the Constrained live endpoint the
// client's setup message is IGNORED, so this module is the single source of
// truth for the live session's tools.
